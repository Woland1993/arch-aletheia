...
# Move user-added backgrounds from Aletheia theme folders to user config
ALETHEIA_DIR="$HOME/.local/share/aletheia"
USER_BACKGROUNDS_DIR="$HOME/.config/aletheia/backgrounds"

if [[ -d $ALETHEIA_DIR/themes ]]; then
  cd "$ALETHEIA_DIR"

  # Get list of git-tracked background files (relative to aletheia dir)
...
THEMES_DIR="$HOME/.config/aletheia/themes"
CURRENT_THEME_LINK="$HOME/.config/aletheia/current/theme"
...
elif [[ -f $HOME/.config/aletheia/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/aletheia/current/theme.name")
fi

# Remove all symlinks from ~/.config/aletheia/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  aletheia-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  aletheia-theme-set "Tokyo Night"
fi
