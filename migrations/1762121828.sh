...
# https://github.com/Woland1993/arch-aletheia/issues/1852
...
aletheia-pkg-add xdg-terminal-exec
...
if command -v alacritty > /dev/null 2>&1; then
  cp "$ALETHEIA_PATH/applications/Alacritty.desktop" ~/.local/share/applications/
fi
...
if [[ -f $waybar_config ]]; then
  sed -i 's|"on-click-right": "aletheia-launch-terminal"|"on-click-right": "xdg-terminal-exec"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL --class=Wiremix -e wiremix"|"on-click": "xdg-terminal-exec --app-id=com.aletheia.Wiremix -e wiremix"|' "$waybar_config"
  aletheia-state set restart-waybar-required
fi

# Update hyprland window rules to use DNS-format class names
system_conf=~/.config/hypr/apps/system.conf
if [[ -f $system_conf ]]; then
  if grep -q 'class:(.*|Impala|' "$system_conf" || grep -q 'class:(.*|Wiremix|' "$system_conf" || grep -q '|Aletheia|' "$system_conf"; then
    sed -i 's/\bImpala\b/com.aletheia.Impala/g; s/\bWiremix\b/com.aletheia.Wiremix/g; s/|Aletheia|/|com.aletheia.Aletheia|/g' "$system_conf"
  fi
fi
