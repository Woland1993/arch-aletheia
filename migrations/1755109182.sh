...
# Users can still opt-in to Cloudflare DNS using: aletheia-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-aletheia-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-aletheia-dns.network
  ...
  echo "To use Cloudflare DNS, run: aletheia-setup-dns Cloudflare"
fi