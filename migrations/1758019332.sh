...
aletheia-refresh-config uwsm/default
aletheia-refresh-config uwsm/env
aletheia-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, aletheia-launch-editor' ~/.config/hypr/bindings.conf
fi
